...
function fetch_user_chat_history($from_user_id, $to_user_id, $connect)
{
 ...
 foreach($result as $row)
 {
  ...
  if($row["from_user_id"] == $from_user_id)
  {
   if($row["status"] == '2')
   {
    $chat_message = '<em>Esta mensagem foi removida</em>';
    $user_name = '<b class="text-success">Você</b>';
   }
   else
   {
    $chat_message = $row['chat_message'];
    $user_name = '<button type="button" class="btn btn-danger btn-xs remove_chat" id="'.$row['chat_message_id'].'">x</button>&nbsp;<b class="text-success">Você</b>';
   }
